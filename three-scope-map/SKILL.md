...
18. Preserve the complete Earth intro timeline when optimizing the Earth-to-map handoff. On a cold start, warm the Earth scene first, emit `scene-ready`, prepare one complete inactive destination frame while the Earth canvas is still hidden, and start the visible Earth intro only after that frame is ready. Reveal the static destination frame during handoff, and start its continuous animation loop only after Earth postprocessing/fade is effectively complete.
...
- Confirm the Earth intro remains intact, the handoff first reveals a compiled static destination frame, and the destination continuous animation loop begins only after Earth fade/postprocessing completes.
