...
export function createScopeMap(
  ...
  let activeScope: MapScope = initialMapState.scope;
  ...
  function waitForBuildSlice() {
    return active ? waitForNextFrame() : waitForPreloadSlice();
  }
  ...
  function updateSouthSeaInsetSize() {
    ...
    if (southSeaInsetEl && nextWidth !== lastSouthSeaInsetWidth) {
      lastSouthSeaInsetWidth = nextWidth;
      southSeaInsetEl.style.width = `${nextWidth}px`;
    }
  }
  ...
  async function createMap() {
    ...
    for (let featureIndex = 0; featureIndex < renderableFeatures.length; featureIndex += 1) {
      ...
      if (active && featureIndex % 2 === 1) await waitForNextFrame();
      if (!active && featureIndex % 6 === 5) await waitForPreloadSlice();
    }
  ...
  async function warmInitialMapRenderer(group: THREE.Group, buildVersion: number) {
    ...
    if (!active) settleMapForStaticFrame(group);
    warmRenderer.render(warmScene, warmCamera);
    if (!active) labelRenderer?.render(warmScene, warmCamera);
  ...
  async function rebuildMapForCurrentState() {
    if (!scene) return;
    activeScope = currentState.scope;
    updateSouthSeaInsetVisibility();
  ...
  function setActive(value: boolean) {
    ...
    if (active && !wasActive) {
      ...
      resolutionUpgradeHandle = globalThis.setTimeout(() => {
        ...
        if (!active || !renderer || !host) return;
  ...
  function setup() {
    ...
    renderer.setPixelRatio(Math.min(window.devicePixelRatio, active ? 1.5 : 1));
  ...
  function getHostSize() {
    ...
    const width = Math.max(1, Math.round(host?.clientWidth || rect?.width || 1920));
    const height = Math.max(1, Math.round(host?.clientHeight || rect?.height || 1080));
    return { width, height };
  }
