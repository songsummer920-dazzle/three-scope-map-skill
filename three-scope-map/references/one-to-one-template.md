...
- approved narrow segmented-ribbon chase light implementation; do not replace it with a generic wide ribbon, unsanitized continuous triangle strip, multiple contour bands, or filled glow surface
...
- Outer-contour chase light reuses the approved one-loop segmented-ribbon implementation with the validated width/segment constants, independent short quads, additive blending, disabled depth test/write, and per-vertex alpha tail. A short `THREE.Line` implementation is acceptable only as the safe fallback when the bundled renderer cannot be reused.
