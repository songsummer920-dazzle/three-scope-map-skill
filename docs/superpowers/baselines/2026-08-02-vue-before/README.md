...
- **`03-cloud-handoff.png` 例外**：这一张已于同日重截，替换了上面 commit 对应的原始产物 —— 见下方「`03-cloud-handoff` 重截记录」一节。
...
| `03-cloud-handoff.png` | 在地球中国大陆区域点击后 ~1.07s（`.china-map-stage` 刚满足 `is-handoff && !is-active`） | **已重截**（见下方专节）：明亮的地球仍完整可见——绿色地形、中国高亮凸起、多条国际飞线、极地扫描环纹，云层揭幕刚起步，尚未产生可见的转黑/遮罩效果。 |
...
## `03-cloud-handoff` 重截记录（2026-08-02，Task 3 收尾修复）

### 为什么重截

原始 `03-cloud-handoff.png` 是用**第一版截图脚本**截的——那版脚本在点击地球后用固定 `sleep(~1.5s)` 起拍。脚本后来被重写成状态门控（等 `.china-map-stage` 同时满足 `is-handoff` 为真、`is-active` 为假，见上面"确定性契约"一节），实测这个门控条件平均在点击后 **~1.07s** 就已经满足——比旧脚本的固定 1.5s 早了约 **400ms**。云层揭幕（`beginChinaHandoff()` → `showChinaMap()`）这段窗口内画面变化很快（从地球仍完全明亮，到云层大幅遮罩转黑），400ms 的提前量足以让两版脚本截到观感完全不同的两帧。

Task 3 审查阶段做过受控实验：把代码回退到重构前（commit `abd6b4b`），用**当前**（状态门控版）脚本重截，与仓库里的旧基线 03 逐像素对比，差异仍高达 **64.94%**（`pctPixelsDiff`，阈值 `|Δchannel|>24` 计），量级与"重构前 vs 重构后"代码对比时 03 的差异（19.14%）不在一个数量级——说明这 64.94% 是脚本触发时机不同造成的伪回归，与被重构的代码无关。旧基线 03 因此判定为**作废**，需要用当前脚本、在重构前代码状态下重截。

### 重截操作与验证

1. 用 `git worktree add /tmp/baseline-wt abd6b4b` 建立重构前代码的独立工作副本（`node_modules` 软链复用，未触碰当前分支工作区），起 `npm run dev` 于 `http://127.0.0.1:5173/`。
2. 用当前 `capture.cjs`（状态门控版）连续跑了 **2 次**，各输出到独立临时目录，只取各自的 03：
   - 两次重截彼此的像素差异：**9.71%**（`pctPixelsDiff`），低于已知的 02 抖动量级（16.13%）——03 是 1.44s 云层揭幕动画里的一个过渡瞬间，本就存在相位抖动，9.71% 判定为可接受范围，不是"这张图不适合做像素基线"的量级（64.94% 那种）。
   - 两次重截各自与旧（作废）基线 03 的差异：63.60% / 64.53%——与 Task 3 审查阶段记录的 64.94% 同一量级，进一步坐实旧基线 03 作废、而非这次重截本身不稳定。
3. 取其中一次重截（`recapture-1`）替换仓库里的 `03-cloud-handoff.png`。
4. 用 Read 工具目视核对新旧两张 03：
   - **旧（作废）03**：画面几乎全黑，仅隐约可见中国轮廓线条透出云雾——这是云层揭幕动画播放到较后段（更接近转黑高潮）的一帧。
   - **新 03**：明亮的地球仍完整可见——绿色地形网格、中国大陆高亮凸起、多条国际飞线弧线、极地扫描环纹清晰，云层揭幕刚起步，肉眼看不出转黑迹象——这是揭幕动画刚触发的最早一帧。
5. 最终判据：用**当前分支 HEAD**（`e33925f`，含重构后代码）在独立端口（`5174`，未占用/未修改任何仓库文件）重新起 dev server 并跑一次截图，把新截的 03 与刚替换进去的新基线 03 逐像素对比：**差异 0.24%**（`pctPixelsDiff`）——落在正常抖动范围内（远低于 9.71%），确认重构后代码与新基线一致，替换有效。

### 结论

`03-cloud-handoff.png` 已重截替换。其余 5 张（01/02/04/05/06）未改动。
