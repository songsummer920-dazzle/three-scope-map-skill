...
export function createEarthView(
  ...
  function enterChina(event: PointerEvent) {
    ...
    if (introValue < 0.98 || isTransitioning || !hostElement || !renderer || !camera || !spinGroup || !globeOrientation) return;
  ...
  function animate() {
    ...
    if (controls && !isTransitioning) controls.enabled = introValue >= 0.98;
    controls?.update();
    ...
    const idleMotionTarget = interactionPausesIdle || isTransitioning
      ? 0
      : ambientMotionReveal;
    ...
    if (chinaOutlineGroup && !isTransitioning) {
